...
const EmployeeProductsPage = () => {
  ...
  const [error, setError] = useState(null);
    ...
    const fetchEmployeeProducts = async () => {
      try {
        setLoading(true);
        const response = await axios.get('/api/products/category/employee-products');
        setProducts(response.data.data.products);
      } catch (err) {
        console.error('Error fetching employee products:', err);
          ...
          <p className="text-sm text-gray-600 mb-4 line-clamp-2 leading-relaxed">{product.shortDescription}</p>
             ...
             <div className="space-y-2 text-sm">
               <div className="flex items-center justify-between">
                 <span className="text-blue-700 font-semibold">Suitable for:</span>
                 <span className="text-blue-600 font-medium">
                   {product.specifications?.find(spec => spec.name === 'Suitable for')?.value || 'Employee Recognition'}
                 </span>
               </div>
               <div className="flex items-center justify-between">
                 <span className="text-blue-700 font-semibold">Customization:</span>
                 <span className="text-blue-600 font-medium">
                   {product.specifications?.find(spec => spec.name === 'Customization')?.value || 'Available'}
                 </span>
               </div>
             </div>
           </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="flex items-center space-x-3 mb-4">
            ...
            <div>
              <h1 className="text-4xl font-black text-gray-900">Employee Products</h1>
              <p className="text-purple-600 font-semibold">Recognition and appreciation solutions for your team</p>
            </div>
          </div>
        </motion.div>
        ...
        >
          <h2 className="text-2xl font-black text-gray-900 mb-6 text-center">Employee Recognition Solutions</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="text-center">
              ...
              <h3 className="text-lg font-bold text-gray-900 mb-2">Recognition Programs</h3>
              <p className="text-gray-600">Complete solutions for employee of the month and milestone awards</p>
            </div>
            <div className="text-center">
              <div className="w-16 h-16 bg-gradient-to-r from-purple-500 to-pink-500 rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-lg">
                <Gift size={24} className="text-white" />
              </div>
              <h3 className="text-lg font-bold text-gray-900 mb-2">Appreciation Gifts</h3>
              <p className="text-gray-600">Thoughtful gifts to show employee care and boost morale</p>
            </div>
            <div className="text-center">
              ...
              <h3 className="text-lg font-bold text-gray-900 mb-2">Performance Awards</h3>
              <p className="text-gray-600">Premium awards for outstanding performance and achievements</p>
            </div>
          </div>
        </motion.div>
           ...
           {loading ? (
             ...
             <div className="text-center py-12">
               ...
               <button
                 ...
                 className="bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white px-6 py-2 rounded-xl font-bold transition-all duration-300"
               >
                 Try Again
               </button>
             </div>
             ...
             <div className="text-center py-12">
               ...
               <p className="text-gray-600">No employee products are currently available.</p>
             </div>
           ) : (
